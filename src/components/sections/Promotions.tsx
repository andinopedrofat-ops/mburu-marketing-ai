import { brand } from "@/config/brand";

const badgeColors: Record<string, string> = {
  yellow: "bg-brand-yellow text-brand-dark",
  blue: "bg-brand-blue-mid text-white",
  green: "bg-whatsapp text-white",
};

export default function Promotions() {
  return (
    <section id="promociones" className="py-20 md:py-28 bg-brand-darker">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-14">
          <span className="text-brand-yellow text-sm font-semibold uppercase tracking-widest">
            Promociones
          </span>
          <h2 className="mt-3 text-4xl md:text-5xl font-black text-white leading-tight">
            La mejor inversión
            <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-yellow to-brand-yellow/50">
              que vas a hacer hoy.
            </span>
          </h2>
          <p className="mt-4 text-gray-400 text-lg max-w-xl mx-auto">
            Ofertas reales para que arrancar no sea una excusa financiera.
          </p>
        </div>

        {/* Promotions grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {brand.promotions.map((promo, i) => {
            const waUrl = `${brand.whatsapp.url}?text=${encodeURIComponent(promo.whatsappMessage)}`;
            return (
              <div
                key={i}
                className={`relative rounded-2xl p-7 flex flex-col transition-all duration-300 hover:scale-[1.02] ${
                  promo.highlight
                    ? "bg-gradient-to-br from-brand-yellow/15 to-brand-yellow/5 border-2 border-brand-yellow/50 shadow-xl shadow-brand-yellow/10"
                    : "bg-brand-card border border-brand-border hover:border-white/20"
                }`}
              >
                {/* Badge */}
                <div className="flex items-start justify-between mb-5">
                  <span
                    className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full ${
                      badgeColors[promo.badgeColor] || badgeColors.yellow
                    }`}
                  >
                    {promo.badge}
                  </span>
                  {promo.highlight && (
                    <span className="text-2xl">⭐</span>
                  )}
                </div>

                {/* Title */}
                <h3
                  className={`text-2xl font-black mb-3 ${
                    promo.highlight ? "text-brand-yellow" : "text-white"
                  }`}
                >
                  {promo.title}
                </h3>

                {/* Description */}
                <p className="text-gray-400 text-sm leading-relaxed flex-1 mb-6">
                  {promo.description}
                </p>

                {/* CTA Button */}
                <a
                  href={waUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-bold text-sm transition-all duration-200 hover:scale-105 ${
                    promo.highlight
                      ? "bg-whatsapp hover:bg-whatsapp-hover text-white"
                      : "bg-brand-dark border border-brand-border hover:border-white/30 text-white"
                  }`}
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                  {promo.cta}
                </a>
              </div>
            );
          })}
        </div>

        {/* Urgency bar */}
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
          <span className="flex items-center gap-2 text-gray-400 text-sm">
            <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
            Las promos tienen cupo limitado.
          </span>
          <span className="text-gray-600 hidden sm:block">·</span>
          <span className="text-gray-400 text-sm">
            Escribinos hoy para asegurar el tuyo.
          </span>
        </div>
      </div>
    </section>
  );
}
